...
m = motorPosition.Motor(50,0,180,150,500) # Motor(freq,minDeg,maxDeg,minPulseLength,maxPulseLength), initializes motor
f = open("encoder.txt","w") # open or create file to write to.

try:
	while(1):
		encoderReadings = d.encoderReader() # calls function that reads encoder and returns count vector with the count
		m.position([0,1,2],[encoderReadings[0],encoderReadings[1],encoderReadings[0]])
